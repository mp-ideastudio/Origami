...
initWebGL() {
                ...
                const baseGeo = new THREE.CylinderGeometry(0.65, 0.85, 0.04, 64);
                ...
                // Photorealistic White Circle Border
                const borderGeo = new THREE.RingGeometry(0.85, 0.95, 64);
                ...
                const initAvatar = (gltf) => {
                    ...
                    // Ensure scale perfectly matches the 1:1 world scale of the Goblin models, scaled down by 50%
                    const s = 0.5; 
...
animate() {
                ...
                if (!this._lastRotBroadcast || (_now - this._lastRotBroadcast) > 50) {
                    ...
                    this.scene.children.forEach(c => {
                        if (c.userData && c.userData.type === 'enemy' && c.userData.hp > 0) {
                            const dx = c.position.x - this.player.x;
                            const dz = c.position.z - this.player.z;
                            const dist = Math.hypot(dx, dz);
                            if (dist <= 15) {
                                radarEnemies.push({ dx, dz, dist });
                            }
                        }
                    });
                ...
                if (this.lanternConeMat) {
                    ...
                    // Sync SpotLight intensity with organic flicker
                    const flicker = 0.95 + 0.05 * Math.sin(lt * 8.3 + 1.7) * Math.sin(lt * 12.1);
                    if (this.lanternLight) this.lanternLight.intensity = 3.0 * flicker;
                    if (this.lanternFill) this.lanternFill.intensity = 0.6 * flicker;
                    if (this.lanternLens) this.lanternLens.material.opacity = 0.8 + 0.2 * flicker;
                }
                                ...
                                model.traverse(n => {
                                    if (n.isMesh && n.material) {
                                        ...
                                        mats.forEach(mat => {
                                            ...
                                            // Removed the mat.transparent = true and mat.opacity = 0.45 override 
                                            // because physical monsters (Yakuza Goblin) should not be holograms.
                                        });
                                    }
                                });
                ...
                const coneMat = new THREE.ShaderMaterial({
                    ...
                    fragmentShader: `
                        ...
                        void main() {
                            ...
                            float flicker = 0.95 + 0.05 * sin(uTime * 8.3);
