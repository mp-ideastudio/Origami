...
export class DungeonMaster {
  ...
  loadDungeon(dungeonId) {
      ...
      if (!dungeon) {
          console.error(`[DMC] Dungeon ${dungeonId} not found! Falling back to generator.`);
          this.generateNewLevel();
          return;
      }
  ...
  getAvailableActions(room) {
    ...
    Object.values(this.currentLevel.rooms).forEach(r => {
        if (r.monsters && r.monsters.length > 0) {
            ...
            if (dist <= 7.5) { // Within 6 tiles (7.5 world units)
                r.monsters.forEach(mob => {
                    if (mob.state === AI_STATES.HOSTILE) {
                        ...
                        if (monsterDist > 0.1) { // Avoid division by zero
                        ...
                        actions.push({ label: `Command ${mob.name}`, command: `command ${mob.id}` });
                    }
                });
            }
        }
    ...
    if (room.items) {
      room.items.forEach(item => {
        actions.push({ label: `Take ${item.name}`, command: `take ${item.id}` });
      });
    }
  ...
  handleTake(targetId) {
    ...
    if (itemIndex !== undefined && itemIndex !== -1) {
      ...
      this.triggerEvent("item_taken", { roomId: room.id, itemId: item.id, itemIndex: itemIndex });
      const actions = this.getAvailableActions(room);
      this.ui.renderGuideButtons(actions);
    } else {
  ...
  handlePacify(targetId) {
    ...
    const mob = room.monsters?.find(m => m.id === targetId || m.name.toLowerCase().includes(targetId));
  ...
  handleLook(hit) {
      ...
      if (type === 'wall') {
          this.ui.logHTML("<p>You see a sturdy wall, reinforced with ancient wood.</p>");
      } else if (type === 'mob') {
          const roomId = parts[1];
          const mobIndex = parseInt(parts[2]);
          const room = this.store.state.currentRoom;
          
          if (room && room.id === roomId && room.monsters && room.monsters[mobIndex]) {
              const mob = room.monsters[mobIndex];
              const status = mob.state === 'HOSTILE' ? 'hostile' : 'peaceful';
              this.ui.logHTML(`<p>You see a <strong>${mob.name}</strong>. It appears ${status}.</p>`);
          } else {
          ...
          if (room && room.id === roomId && room.items && room.items[itemIndex]) {
              const item = room.items[itemIndex];
              this.ui.logHTML(`<p>You see a <strong>${item.name}</strong> lying on the ground.</p>`);
          }
      } else if (type === 'label') {
      ...
      if (verb === 'dig') {
          ...
          if (type === 'mob') {
              ...
              if (room && room.id === roomId && room.monsters && room.monsters[mobIndex]) {
                  ...
                  this.ui.logHTML(`<p class="damage-text">The ${mob.name} is defeated!</p>`);
  ...
  handleFloorClick(point) {
      ...
      const endX = Math.round(point.x);
      const endZ = Math.round(point.z);
      
      // 2. Get Player Position
      const playerRoom = this.store.state.currentRoom;
      if (!playerRoom) return;
      
      const startX = playerRoom.x; // Assuming room coordinates are grid coordinates
      const startZ = playerRoom.z;
      
      console.log(`[DMC] Pathfinding from (${startX},${startZ}) to (${endX},${endZ})`);
      
      // 3. Find Path (BFS)
      const path = this.findPath(startX, startZ, endX, endZ);
      
      if (path && path.length > 0) {
          // 4. Visualize Path
          this.triggerEvent("show_path", { points: path });
          
          // 5. Start Auto-Walk
          this.startAutoWalk(path);
      } else {
  ...
  findPath(startX, startZ, endX, endZ) {
      ...
      while (queue.length > 0 && iterations < 1000) {
          ...
          for (const n of neighbors) {
              const key = `${n.x},${n.z}`;
              if (!visited.has(key)) {
                  // Check if walkable (Room exists at these coords)
                  // We need to check all rooms in the level to see if one exists at (x,z)
                  // This is slow if we iterate all rooms. Better if we had a grid map.
                  // For now, let's iterate.
                  const isWalkable = Object.values(this.currentLevel.rooms).some(r => Math.round(r.x) === n.x && Math.round(r.z) === n.z);
  ...
  triggerEvent(eventName, data) {
      ...
      window.dispatchEvent(new CustomEvent("origami:engine", { detail: { type: eventName, data: data } }));
  }
}
