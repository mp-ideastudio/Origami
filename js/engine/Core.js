                ...
                window.addEventListener('message', (e) => {
                    ...
                    if (e.data.type === 'FPV_ACTION') {
                        ...
                        const SPELL_MAP = {
                            ...
                            'TIDE':       { el:'WATER', color:0x0d6efd, dmg:20, trauma:0.5, label:'\uD83C\uDF0A TIDE',           range: 4.0 },
                            'SURGE':      { el:'WATER', color:0x0d6efd, dmg:30, trauma:0.6, label:'\uD83C\uDF0A SURGE',          range: 5.0 },
                            'HEAL POTION':     { el:'SCROLL',  color:0x00e676, dmg:-20, trauma:0.0, label:'\uD83E\uDDEA POTION (HEAL)', range: Infinity },
                            ...
                            'WAND OF FIREBALLS': { el:'FIRE',  color:0xff4400, dmg:50, trauma:0.9, label:'\uD83D\uDD25 WAND OF FIREBALLS', range: 12.0 },
                            'WAND OF MAGIC MISSILES': { el:'MISSILE', color:0xaa44ff, dmg:30, trauma:0.4, label:'\u2728 MAGIC MISSILES', range: 15.0 },
                            'TOMMY GUN': { el:'MISSILE', color:0xffdd00, dmg:60, trauma:0.8, label:'\uD83D\uDD2B TOMMY GUN', range: 20.0 }
                        };
                            ...
                            if (child.userData?.type === 'enemy' && !child.userData.isDead) {
                                ...
                                if (dist2D < 1.1) {
                                    if (b.userData.isShuriken || b.userData.type === 'SHURIKEN') {
                                        ...
                                        flash.position.copy(child.localToWorld(new THREE.Vector3(hitX, hitY, hitZ)));
                                        ...
                                        if (!b.userData.hitEnemies.has(child.userData.id)) {
                                            ...
                                            // Explode after hitting 3 monsters
                                            if (b.userData.hitEnemies.size >= 3) {
                                                this.spawnRockExplosion(b.position.clone());
                                                this.scene.remove(b);
                                                this.boulders.splice(bi, 1);
                                                bHitEntity = true;
