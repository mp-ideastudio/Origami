         ...
         * NewOrigamiAi.html
         * 
         * This script acts as a headless WebWorker/Iframe container that calculates
         * pathfinding, states, and behavior for all non-player entities in the game.
         * 
         * It listens to messages from NewOrigamiGame.html (The Core Engine), updates 
         * entity states, and sends back visual commands (Move, Attack, Talk).
         */
        ...
        class EntitiesManager {
            constructor() {
                ...
                window.addEventListener('message', (e) => this.handleEngineMessage(e));
                
                // Tick AI is now turn-based and driven by handleEngineMessage
            }
            ...
            initEntities(spawns) {
                ...
                console.log("[AI Brain] Initialized Entities:", this.entities);
            }
                    ...
                    if (entity.type === 'monster') {
                        ...
                        if (entity.state === 'IDLE') {
                            ...
                            if (!this.globalHostility && dist > 8) {
                                ...
                                updates.push({ id: entity.id, action: 'ATTACK' });
                            } else {
                                // Move towards player
                                // 75% Speed Accumulator means they sometimes skip a turn, but we make them always move
                                entity.moveEnergy += 1.0; 
                                if (entity.moveEnergy >= 1) {
                                    entity.moveEnergy -= 1;
                                    const path = this.findPath({x: entity.x, z: entity.z}, {x: this.playerPos.x, z: this.playerPos.z});
                                    if (path && path.length > 0) {
                                        // Stop exactly 1 tile away so we don't walk ONTO the player
                                        if (path.length > 1) {
                                            entity.x = path[0].x;
                                            entity.z = path[0].z;
                                            updates.push({ id: entity.id, action: 'MOVE', x: entity.x, z: entity.z, state: 'HOSTILE' });
                                        } else {
                                            // 1 tile away, we attack
                                            updates.push({ id: entity.id, action: 'ATTACK' });
                                        }
                                    }
                                }
            ...
            applyDamage(id, amt) {
                ...
                e.hp -= amt;
                console.log(`[AI Brain] Entity ${id} took ${amt} damage. HP is now ${e.hp}`);
