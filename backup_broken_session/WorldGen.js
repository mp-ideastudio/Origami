...
createDarkWoodRafterTexture() {
                ...
                ctx.fillStyle = "#150B08"; // Very dark brown wood
                ctx.fillRect(0, 0, size, size);
...
buildWorldGeometry() {
                ...
                const sharedWoodTex = new THREE.TextureLoader().load('assets/textures/Wall.Panel.Basic.1.png');
                sharedWoodTex.wrapS = THREE.RepeatWrapping;
                sharedWoodTex.wrapT = THREE.RepeatWrapping;
                sharedWoodTex.repeat.set(1, 1);
                ...
                const mats = {
                    ...
                    pipWall: new THREE.MeshStandardMaterial({ color: 0x222222, emissive: 0x666666, transparent: true, opacity: 0.35, depthWrite: false }),
                    wallTop: new THREE.MeshStandardMaterial({ map: wallTopTex, color: 0xE6E6E6, emissive: 0x1A1A1A, roughness: 0.9, metalness: 0.0 }),
                    ...
                    monster: new THREE.MeshPhysicalMaterial({ 
                        ...
                        emissiveIntensity: 0.8, // Slightly boosted to combat flashlight washout
                        map: objTex,
                        roughness: 0.1,
                        metalness: 0.1,
                        transparent: true,
                ...
                // Generate 8 wall materials for the procedural walls
                const wallTex = new THREE.TextureLoader().load('assets/textures/Wall.Panel.Basic.1.png');
                wallTex.wrapS = THREE.RepeatWrapping;
                wallTex.wrapT = THREE.RepeatWrapping;
                wallTex.repeat.set(1, 1); // Stretched instead of double-stacking

                const wallTierMats = [];
                for(let i=0; i<8; i++) {
                    wallTierMats.push(new THREE.MeshStandardMaterial({ 
                        map: wallTex, 
                        color: 0x444455, // Dark moody blue-grey
                        emissive: 0x0c0c0c, // 5% background luminosity
                        roughness: 0.9, 
                        metalness: 0.1 
                    }));
                }
                ...
                for(let i=0; i<8; i++) {
                    ...
                    mesh.castShadow = true; // RE-ENABLED FOR AAA ISOMETRIC SHADOWS
                    mesh.receiveShadow = true;
                    mesh.frustumCulled = false;
                    mesh.layers.set(1); // Force FPV and Isometric only
                    fpvWallMeshes.push(mesh);
                }

                // pipInstancedWalls completely removed as we now use true 3D FPV walls for Isometric Top-Down view
                ...
                // Ceiling
                const ceilingTex = new THREE.TextureLoader().load('assets/textures/Wall.Panel.Basic.1.png');
                ceilingTex.wrapS = THREE.RepeatWrapping;
                ceilingTex.wrapT = THREE.RepeatWrapping;
                ceilingTex.repeat.set(this.mapWidth, this.mapHeight);
                mats.ceil.map = ceilingTex;
                ...
                // Add random 3D ceiling rafters crossing the rooms
                const numRafters = Math.floor((this.mapWidth * this.mapHeight) / 20); // Scale with dungeon size
                for (let i = 0; i < numRafters; i++) {
                    const rx = Math.floor(Math.random() * this.mapWidth);
                    const rz = Math.floor(Math.random() * this.mapHeight);
                    // Only place in playable areas
                    if (this.mapData[rx]?.[rz]?.type !== 'wall') {
                        const rafterGeo = new THREE.BoxGeometry(this.gridSize * (Math.random() * 4 + 2), 0.3, 0.4);
                        const rafter = new THREE.Mesh(rafterGeo, mats.ceil);
                        rafter.position.set(rx * this.gridSize, wallHeight - 0.15, rz * this.gridSize);
                        rafter.rotation.y = Math.random() > 0.5 ? 0 : Math.PI / 2;
                        rafter.layers.set(2); // FPV only
                        this.worldGroup.add(rafter);
                    }
                }
                ...
                // --- Floor Gas (Horror Movie Smoke) ---
                const gasCanvas = document.createElement('canvas');
                gasCanvas.width = 128; gasCanvas.height = 128;
                const gasCtx = gasCanvas.getContext('2d');
                const grd = gasCtx.createRadialGradient(64,64,0, 64,64,64);
                grd.addColorStop(0, 'rgba(120, 150, 180, 0.4)');
                grd.addColorStop(0.5, 'rgba(120, 150, 180, 0.15)');
                grd.addColorStop(1, 'rgba(120, 150, 180, 0)');
                gasCtx.fillStyle = grd;
                gasCtx.fillRect(0,0,128,128);
                const gasTex = new THREE.CanvasTexture(gasCanvas);
                
                const gasGeo = new THREE.PlaneGeometry(this.gridSize * 15, this.gridSize * 15);
                ...
                this.floorGasMeshes = [];
                for(let i=0; i<3; i++) { // Reduced from 12 to 3 to prevent extreme fill-rate overdraw and FPS drops
                    const gasMesh = new THREE.Mesh(gasGeo, gasMat);
                    gasMesh.rotation.x = -Math.PI / 2;
                    gasMesh.position.set(
                        (Math.random() * this.mapWidth) * this.gridSize,
                        1.0, // Just above the floor
                        (Math.random() * this.mapHeight) * this.gridSize
                    );
                    gasMesh.layers.enable(1); // Top-down
                    gasMesh.layers.enable(2); // FPV ceiling
                    gasMesh.layers.enable(3); // PiP
                    this.worldGroup.add(gasMesh);
                    this.floorGasMeshes.push({
                        mesh: gasMesh,
                ...
                const buildEntity = (sp, isMon, mesh) => {
                    ...
                    if (isMon) {
                        ...
                        // Tactical Monster Light Source removed to fix FPS drops and shader recompilation lag
                            ...
                            if (child.isMesh && child.material) {
                                ...
                                mats.forEach(m => {
                                    ['map', 'normalMap', 'emissiveMap', 'roughnessMap', 'metalnessMap'].forEach(key => {
                                        if (m[key] && m[key].image === undefined) m[key] = null;
                                    });
                                });
                            }
                        });
                ...
                this.mobSpawns.forEach((sp, idx) => {
                    ...
                    gltfLoader.load(targetModel, (gltf) => {
                                ...
                                mats.forEach(m => {
                                    ['map', 'normalMap', 'emissiveMap', 'roughnessMap', 'metalnessMap'].forEach(key => {
                                        if (m[key] && m[key].image === undefined) {
                                            console.warn('Stripping broken monster texture:', key);
                                            m[key] = null;
                                        }
                                    });
                                });
                        ...
                        // High-performance 'fake' light to simulate floor illumination without killing FPS
                        const fakeLightCanvas = document.createElement('canvas');
                        fakeLightCanvas.width = 64; fakeLightCanvas.height = 64;
                        const ctxL = fakeLightCanvas.getContext('2d');
                        const grdL = ctxL.createRadialGradient(32, 32, 0, 32, 32, 32);
                        grdL.addColorStop(0, 'rgba(255, 68, 51, 0.5)'); // 0xff4433 glowing center
                        grdL.addColorStop(1, 'rgba(255, 68, 51, 0)');
                        ctxL.fillStyle = grdL;
                        ctxL.fillRect(0,0,64,64);
                        
                        const fakeLightGeo = new THREE.PlaneGeometry(18, 18);
                        const fakeLightMat = new THREE.MeshBasicMaterial({
                            map: new THREE.CanvasTexture(fakeLightCanvas),
                            transparent: true,
                            blending: THREE.AdditiveBlending,
                            depthWrite: false
                        });
                        const monsterLight = new THREE.Mesh(fakeLightGeo, fakeLightMat);
                        monsterLight.rotation.x = -Math.PI / 2;
                        monsterLight.position.set(0, 0.15, 0); // Just above the floor
                        monsterLight.layers.enable(1); // Top-down
                        monsterLight.layers.enable(3); // PiP
                        entityWrapper.add(monsterLight);
                        ...
                        if (gltf.animations && gltf.animations.length > 0) {
                            ...
                            const bowClip    = _findClip(/bow|greet|salute/);
                            ...
                            entityWrapper.userData.bowAction    = bowClip    ? mixer.clipAction(bowClip)    : null;
                        ...
                        goblin.traverse((child) => {
                            if (child.isMesh) {
                                ...
                                // Apply v21.2 Hologram Glass Ethereal Ghost Material
                                if (!isEye) {
                                    nativeMat.transparent = true;
                                    nativeMat.opacity = 0.40; // Ghostlike ethereal transparency
                                    
                                    // Remove DoubleSide because it absolutely tanks FPS on high-poly models
                                    nativeMat.side = THREE.FrontSide;
                                    
                                    // Preserve native vertex colors and textures, just add a spectral cyan tint
                                    if (nativeMat.emissive !== undefined) {
                                        nativeMat.emissive.setHex(0x00ffcc); // Teal/Cyan ethereal glow
                                        nativeMat.emissiveIntensity = 0.6;
                                    }
                                    
                                    nativeMat.depthWrite = false;
                                    nativeMat.blending = THREE.NormalBlending;
                                    nativeMat.needsUpdate = true;
                                    child.material = nativeMat;
                                } else {
                                    // DONT REFLECT LIGHT FROM EYES - Use pure unlit white material
                ...
                if (this.entrancePos) {
                    this.spawnLootCard = (x, z, catId, inTitle, desc, kanji, attr, isShopItem = false, price = 50) => {
                        ...
                        // No dynamic light source added here! 
                        // Adding/removing PointLights during gameplay forces all scene shaders to recompile,
                        // causing massive 5-10s FPS drops when picking up a loot card.
                        ...
                        // Loot Light Source removed to prevent massive 5-10s FPS drop on pickup
                        // (Shader recompilation triggered by removing lights)
