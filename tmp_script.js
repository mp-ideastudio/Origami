    ...
    <style>
        ...
        /* Movement Controls - Fluid Golden Placement */
        #movement-pad {
            position: absolute;
            z-index: 50;
            display: grid;
            grid-template-areas: 
              ...
              ". down .";
            gap: 1.5vmin;
            /* Golden minor anchoring for bottom left */
        ...
        /* Fluid overlay for UI adhering to Golden Ratio height limit */
        #ui-frame {
            bottom: 0px;
            left: 0px;
            width: 100%;
            height: clamp(300px, 61.8vh, 1200px); /* Primary Golden Ratio rule for UI prominence */
        }

        #ai-frame { display: none; } /* Strictly headless logic */

        /* Interaction prompt (temporary center screen) */
        #interaction-prompt {
        ...
        #ui-frame { pointer-events: none; display: none !important; }

        /* Realtime Tuning Panel */
        #tuning-panel {
            display: none;
            position: absolute;
            top: 15px;
        ...
        #fps-counter {
            ...
            background: rgba(0,0,0,0.6); color: #00ffcc; padding: 4px 12px; border-radius: 12px;
            border: 1px solid rgba(0, 255, 204, 0.3); font-family: monospace; font-size: 0.9rem;
            z-index: 90; text-shadow: 0 1px 3px rgba(0,0,0,0.8); pointer-events: none;
            -webkit-backdrop-filter: blur(4px); backdrop-filter: blur(4px);
        }

        #turn-counter {
            position: absolute; top: 15px; right: 15px; 
            background: rgba(0,0,0,0.6); color: #ffcc00; padding: 6px 16px; border-radius: 6px;
            border: 1px solid rgba(255, 204, 0, 0.4); font-family: 'Inter', sans-serif; font-size: 1.1rem;
            z-index: 90; font-weight: 800; letter-spacing: 0.1rem;
            box-shadow: 0 4px 15px rgba(0,0,0,0.5); pointer-events: none;
            -webkit-backdrop-filter: blur(4px); backdrop-filter: blur(4px);
            text-transform: uppercase;
        }
        /* Floating Combat Text (WoW Style) */
        #fct-container {
            position: absolute;
            top: 0; left: 0;
            width: 100%; height: 100%;
        ...
        .combat-text {
            ...
            animation: fctFloatUp 1.2s cubic-bezier(0.25, 0.8, 0.25, 1) forwards;
        ...
        @keyframes fctFloatUp {
            ...
            30% { transform: translate(-50%, -65%) scale(1.0); opacity: 1; }
            80% { transform: translate(-50%, -100%) scale(1.0); opacity: 0.8; }
            100% { transform: translate(-50%, -120%) scale(0.8); opacity: 0; }
        }

        /* Asset Preloader - High-End / Next-Gen */
        #loading-screen {
            position: absolute; top: 0; left: 0; width: 100%; height: 100%;
            background-color: #0b0b0e; z-index: 9999; display: flex; flex-direction: column;
            align-items: center; justify-content: center;
            transition: opacity 0.4s ease-out;
        }
        .loader-content { text-align: center; color: #00ffff; text-shadow: 0 0 10px rgba(0,255,255,0.5); font-family: 'Inter', sans-serif; letter-spacing: 0.2rem; }
        .loader-content h2 { font-weight: 800; margin-bottom: 2rem; font-size: 1.5rem; text-transform: uppercase; }
        .progress-box { width: 350px; height: 6px; background: rgba(0, 255, 255, 0.1); border: 1px solid rgba(0, 255, 255, 0.2); border-radius: 3px; overflow: hidden; margin: 0 auto; box-shadow: 0 0 15px rgba(0,255,255,0.1) inset; }
        #loading-bar-fill { width: 0%; height: 100%; background: linear-gradient(90deg, #0088ff, #00ffff); box-shadow: 0 0 15px #00ffff; transition: width 0.1s ease-out; }
        #loading-text { margin-top: 1rem; font-size: 0.9rem; font-weight: 600; font-family: monospace; }
        #loading-file { margin-top: 0.5rem; font-size: 0.65rem; color: rgba(0, 255, 255, 0.6); letter-spacing: 0.05rem; font-family: monospace; }
        /* Visceral Combat Overlay */
        .combat-dmg-text {
            position: absolute;
            z-index: 1000;
        ...
        .slash-mark {
            ...
            width: 6px;
            border-radius: 3px;
            background: linear-gradient(180deg, #ff0000 0%, rgba(180,0,0,0.2) 100%);
            box-shadow: 0 0 12px 3px rgba(255,0,0,0.6), 0 0 3px rgba(255,255,255,0.4);
            transform-origin: top center;
            animation: slashDrop 0.55s cubic-bezier(0.22, 0.61, 0.36, 1) forwards;
        }
        @keyframes slashDrop {
            0%   { opacity: 0; transform: translate(-50%,-50%) scaleY(0) rotate(var(--r)); }
            15%  { opacity: 1; transform: translate(-50%,-50%) scaleY(1) rotate(var(--r)); }
            60%  { opacity: 0.9; }
            100% { opacity: 0;  transform: translate(calc(-50% + var(--tx)), calc(-50% + var(--ty))) scaleY(1.1) rotate(var(--r)); }
        }

        /* Blood vignette that fires when player takes damage */
        #blood-vignette {
            position: absolute; top: 0; left: 0; width: 100%; height: 100%;
            pointer-events: none; z-index: 4998;
        ...
        /* Vertical zoom slider overlay for mapview */
        #mapview-container .zoom-slider-container {
        ...
        #map-zoom-slider::-webkit-slider-thumb {
            ...
            background: #00ffcc; border: 1px solid rgba(0,0,0,0.8);
...
<body>
    ...
    <div id="game-container">
        ...
        <!-- Dedicated Layer for WoW style Combat Text -->
        <div id="fct-container"></div>
        <!-- Goblin Slash Hit VFX Overlay -->
        <div id="slash-overlay"></div>
        <!-- Blood vignette (player takes damage) -->
        ...
        <!-- UI Panels Layer (Loaded via Iframe to isolate CSS/JS overhead) -->
        <iframe id="ui-frame" title="Game UI Interface" class="logic-frame" src="NewOrigami.Panels.html" sandbox="allow-scripts allow-same-origin allow-modals" allowtransparency="true"></iframe>
        
    <!-- Realtime Tuning Panel -->
    <div id="tuning-panel">
        <h3>Movement Tuning</h3>
        <div class="tune-row">
            <label><span>Speed (W)</span><span id="val-speed">2.56</span></label>
            <input type="range" id="tune-speed" title="Speed" min="0.5" max="6.0" step="0.01" value="2.56">
        </div>
        <div class="tune-row">
            <label><span>Down-step (Bob)</span><span id="val-bob">0.045</span></label>
            <input type="range" id="tune-bob" title="Bob Amplitude" min="0.0" max="0.3" step="0.001" value="0.045">
        </div>
        <div class="tune-row">
            <label><span>Sway (Shift)</span><span id="val-sway">0.015</span></label>
            <input type="range" id="tune-sway" title="Sway Amplitude" min="0.0" max="0.1" step="0.001" value="0.015">
        </div>
    </div>

    <!-- AI Brain Layer (Headless) -->
        ...
        const Engine = {
            ...
            turnCount: 0,    // Hybrid turn counter
            actionTimePool: 0, // Injected time pool when an action occurs
            ...
            generateMap() {
                ...
                // Carve 1-Wide 7-Tile Entrance Hallway South (5ft corridor)
            ...
            initWebGL() {
                ...
                // Warm key directional light for depth and shadows, radiating down from above
                ...
                // PerspectiveCamera with tight FOV creates the real Diablo 3D depth look
                this.topDownCamera = new THREE.PerspectiveCamera(40, 1, 1, 500);
                ...
                borderMesh.layers.set(1); borderMesh.layers.enable(3);
                ...
                baseMesh.layers.set(1); baseMesh.layers.enable(3);
                ...
                noseMesh.position.set(0, 0.03, -0.6); // Pushed to front edge
                noseMesh.layers.set(1); noseMesh.layers.enable(3);
                ...
                const initAvatar = (gltf) => {
                    ...
                    // Scale uniformly to roughly human scale
                    const box = new THREE.Box3().setFromObject(avatarMesh);
                    const currentHeight = Math.max(1e-6, box.max.y - box.min.y);
                    const s = 1.3 / currentHeight; 
                    ...
                    avatarMesh.traverse((child) => {
                        if (child.isMesh) {
                            child.layers.set(3); 
                            // MeshLambertMaterial restores dimensional shadows and rim lighting depth so it stops looking "wonky"
            ...
            createDungeonTopTexture() {
                ...
                // Solid premium dark slate base (Seamless)
                ctx.fillStyle = "#1a1c1e"; 
                ctx.fillRect(0, 0, 512, 512);

                // Subtle noise for uniform matte texture
                ctx.fillStyle = "rgba(0,0,0,0.15)";
                for (let i = 0; i < 4000; i++) {
                    ctx.fillRect(Math.random() * 512, Math.random() * 512, 2, 2);
                }
                
                // Purely seamless, no borders, no bevels, no crosshairs.
            ...
            buildWorldGeometry() {
                ...
                const mats = {
                    ...
                    wall: new THREE.MeshStandardMaterial({ map: wallTex, color: 0xFFFFFF, roughness: 0.8, metalness: 0.0 }),
                    pipWall: new THREE.MeshStandardMaterial({ color: 0x111111, transparent: true, opacity: 0.15, depthWrite: false }),
                    wallTop: new THREE.MeshStandardMaterial({ map: wallTopTex, color: 0xFFFFFF, roughness: 0.9, metalness: 0.0 }),
                ...
                const wallMaterials = [
                    mats.wall,    // Right
                    mats.wall,    // Left
                    mats.wallTop, // Top (Custom Neumorphic Dungeon Top)
                    mats.wall,    // Bottom
                    mats.wall,    // Front
                    mats.wall     // Back
                ];
                ...
                instancedWalls.layers.set(1); // Force FPV only
                ...
                // Removed per user request to maintain clean top-down map aesthetic without brown blockers!
                ...
                for (let x = -PADDING; x < this.mapWidth + PADDING; x++) {
                    for (let z = -PADDING; z < this.mapHeight + PADDING; z++) {
                        ...
                        if (type === 'wall') {
                            ...
                            pipInstancedWalls.setMatrixAt(wallIdx, dummy.matrix);
                ...
                // --- ROOM NUMBERS (Samurai Dojo Theme) ---
                if (this.rooms) {
                ...
                if (this.entrancePos) {
                    this.spawnLootCard = (x, z, catId, inTitle, desc, kanji, attr, isShopItem = false, price = 50) => {
                        ...
                        const createLootIcon = () => {
                            ...
                            if (catId === 'EARTH') {
                                ...
                                iconMesh.add(die);
                                customUpdate = (t) => { iconMesh.rotation.y = t * 1.5; iconMesh.rotation.x = Math.sin(t * 2) * 0.5; };
                            }
                        ...
                        // Fully 3D model exactly matching the UI panels
                        const EXACT_SCALE = 0.07;
                        
                        // Set the model directly at Z=0 so it physically pierces the card symmetrically
                        iconModel.iconMesh.scale.set(EXACT_SCALE, EXACT_SCALE, EXACT_SCALE);
                        iconModel.iconMesh.position.set(0, -0.096, 0);
                        iconWrapper.add(iconModel.iconMesh);
                        
                        cardGroup.add(iconWrapper);
                        
                        const combinedUpdate = (t) => {
                            if (iconModel.customUpdate) iconModel.customUpdate(t);
                        };
                        ...
                        cardGroup.userData = { 
                            ...
                            cardDataURL: canvas.toDataURL(), iconMesh: iconWrapper, customUpdate: combinedUpdate,
                    ...
                    // Explicitly spawn Samurai Helmet to the left of Room 1 Goblin
                    ...
                    if (!this.level || this.level === 1) {
                        const shop = this.rooms.find(r => r.id === 99);
                        if (shop) {
                            // Lay out cards against the West wall of the shop
                            const shopWestX = shop.x; // the actual floor tiles start at shop.x
                            let zPos = shop.y;
                            for (let i = 0; i < 4; i++) {
            ...
            initControls() {
                ...
                window.addEventListener('pointerdown', (e) => {
                    if (this.activeChat && !this.activeChat.fading) {
                        ...
                        // FPV Raycaster for click-to-move
            ...
            findPath(startX, startZ, endX, endZ) {
                ...
                if (!this.isValidGridSpace(endNode.x, endNode.z)) return [];
                
                let openList = [startNode];
                let closedList = [];
                
                const getH = (curr, end) => Math.abs(curr.x - end.x) + Math.abs(curr.z - end.z);
                const isEq = (a, b) => a.x === b.x && a.z === b.z;
                ...
                while (openList.length > 0 && maxIter-- > 0) {
                    ...
                    for (let n of neighbors) {
                        if (!this.isValidGridSpace(n.x, n.z)) continue;
                        if (closedList.find(c => isEq(c, n))) continue;
                        
                        let gScore = currNode.g + 1;
                        let gScoreIsBest = false;
                        
                        let inOpen = openList.find(o => isEq(o, n));
                        if (!inOpen) {
                            gScoreIsBest = true;
                            n.h = getH(n, endNode);
                            openList.push(n);
                            ...
                            gScoreIsBest = true;
                            n = inOpen; // Update the one already in the list
                        }
            ...
            setWaypointPath(targetGridX, targetGridZ) {
                ...
                const sx = Math.round(this.player.x);
                const sz = Math.round(this.player.z);
            ...
            spawnLoot(gridX, gridZ) {
                ...
                this.spawnLootCard(gridX, gridZ, lc.el, lc.name, lc.desc, lc.kanji, lc.attr);
                window.parent.postMessage({ type: 'LOG_EVENT', text: `You hear a card drop to the floor.`, logType: 'system' }, '*');
            },

            // ─────────────────────────────────────────────────────────────────
            // PROJECTILE: Physics System (Boulders, Shurikens, Arrows)
            // ─────────────────────────────────────────────────────────────────
            spawnProjectile(action, spellData) {
                ...
                if (spellData.el === 'EARTH') {
                    ...
                    if (!this._boulderSharedMat) {
                        ...
                        sCtx.fillStyle = '#888'; sCtx.fillRect(0, 0, 128, 128);
                        for (let i = 0; i < 300; i++) {
                            const px = Math.random() * 128, py = Math.random() * 128, r = Math.random() * 10 + 2, v = Math.floor(80 + Math.random() * 80);
                            sCtx.fillStyle = `rgba(${v},${v - 8},${v - 18},0.55)`; sCtx.beginPath(); sCtx.arc(px, py, r, 0, Math.PI * 2); sCtx.fill();
                        }
                        sCtx.strokeStyle = 'rgba(25,15,5,0.6)'; sCtx.lineWidth = 2;
                        for (let i = 0; i < 12; i++) { sCtx.beginPath(); sCtx.moveTo(Math.random() * 128, Math.random() * 128); sCtx.quadraticCurveTo(Math.random() * 128, Math.random() * 128, Math.random() * 128, Math.random() * 128); sCtx.stroke(); }
                        this._boulderSharedMat = new THREE.MeshStandardMaterial({ map: new THREE.CanvasTexture(sc), roughness: 0.95, metalness: 0.02, color: 0xbbbbaa });
                        this._boulderSharedGeo = new THREE.SphereGeometry(0.55, 10, 10);
                    }
                    projectileMesh.add(new THREE.Mesh(this._boulderSharedGeo, this._boulderSharedMat));
                } else if (spellData.el === 'FIRE') {
                    speed = 18.0;
                    const core = new THREE.Mesh(new THREE.IcosahedronGeometry(0.4, 1), new THREE.MeshStandardMaterial({ color: 0xb71c1c, emissive: 0x4a0000, emissiveIntensity: 3, flatShading: true }));
                ...
                projectileMesh.position.set(startX, 0.8, startZ);
                ...
                projectileMesh.userData = Object.assign(projectileMesh.userData || {}, {
                    ...
                    dmg: spellData ? spellData.dmg : 25,
                    active: true,
                    element: spellData.el,
                    color: spellData.color,
                    trauma: spellData.trauma
                });
                ...
                const anim = () => {
                    ...
                    if (frame < 8) {
                        ...
                        if (target && !target.userData.isDead && this.activeTargetDist <= spellData.range) {
                            ...
                            if (target.userData.monBase && target.userData.monBase.children[1]) {
            ...
            initComms() {
                ...
                window.addEventListener('message', (e) => {
                    ...
                    if (e.data.type === 'PIP_SYNC_RECT') {
                        if (!this._pipRect) this._pipRect = { width: 300, height: 300, left: 0, top: 0, bottom: 0 };
                        this._pipRect.width = e.data.width;
                        this._pipRect.height = e.data.height;
                        this._pipRect.left = e.data.left;
                        this._pipRect.top = e.data.top;
                        this._pipRect.bottom = e.data.bottom;
                        return;
                        ...
                        return;
                    } else if (e.data.type === 'PIP_ROTATE') {
                        this._pipLastInteractionTime = Date.now();
                        if (!this._pipRot) this._pipRot = { theta: 0.5, phi: 0.96, radius: 35 };
                        this._pipRot.theta -= e.data.dx * 0.01;
                        this._pipRot.phi -= e.data.dy * 0.01;
                        this._pipRot.phi = Math.max(0.1, Math.min(Math.PI / 2 - 0.05, this._pipRot.phi));
                        return;
                    } else if (e.data.type === 'WAGER_RESOLVE') {
                        if (e.data.win) {
                            this.player.gold += e.data.amount;
                            window.parent.postMessage({ type: 'LOG_EVENT', logType: 'system', text: `You WON the wager! +${e.data.amount} Gold!` }, '*');
                        } else {
                            this.player.gold -= e.data.amount;
                            window.parent.postMessage({ type: 'LOG_EVENT', logType: 'system', text: `You LOST the wager! -${e.data.amount} Gold.` }, '*');
                        }
                        this.syncPlayerStats();
                        return;
                        ...
                        return;
                    } else if (e.data.type === 'PIP_CLICK') {
                        if (!this.pipCamera) return;
                        this.raycaster.setFromCamera({x: e.data.x, y: e.data.y}, this.pipCamera);
                        const intersects = this.raycaster.intersectObjects(this.worldGroup.children, true);
                        if (intersects.length > 0) {
                            const point = intersects[0].point;
                            this.setWaypointPath(point.x / this.gridSize, point.z / this.gridSize);
                    ...
                    if (e.data.type === 'FPV_ACTION') {
                        ...
                        const SPELL_MAP = {
                            ...
                            'SHURIKEN':{ el:'MISSILE',color:0x888888, dmg:15, trauma:0.2, label:'\u2734 SHURIKEN',range: 6.0 },
                            'SHORT BOW':  { el:'MISSILE',color:0xaa8800, dmg:25, trauma:0.4, label:'\uD83C\uDFF9 SHORT BOW',   range: 10.0 },
                            'LONG BOW':   { el:'MISSILE',color:0xaa6600, dmg:45, trauma:0.7, label:'\uD83C\uDFF9 LONG BOW',    range: 15.0 },
            ...
            animate() {
                ...
                if (this.autoWalkPath && this.autoWalkPath.length > 0) {
                    const targetNode = this.autoWalkPath[0];
                    const dx = targetNode.x - this.player.x;
                    const dz = targetNode.z - this.player.z;
                    ...
                    if (distSq < 0.05) {
                        ...
                        // Remove the corresponding visual from the group
                        if (this.pathVisualsGroup && this.pathVisualsGroup.children.length > 0) {
                            this.pathVisualsGroup.remove(this.pathVisualsGroup.children[0]);
                        }
                        ...
                        this.player.autoTurnTarget = Math.atan2(dx, dz);
                        
                        // Only move forward if we are roughly facing the correct direction
                        const diff = this.player.autoTurnTarget - this.player.rot;
                        let normDiff = Math.atan2(Math.sin(diff), Math.cos(diff));
                        if (Math.abs(normDiff) < 0.5) {
                            moveDir = 1;
                        } else {
                            moveDir = 0; // stop and turn first!
                        }
                    }
                }
                ...
                if ((moveDir !== 0 || this.player.autoTurnTarget !== null) && !this._haltPlayer) {
                    ...
                    if (this.player.autoTurnTarget !== null) {
                        ...
                        // While turning, smoothly slide toward the center of the current tile
                        // This prevents the "forcefield bounce" when entering narrow hallways
                        const tileCenterX = Math.round(this.player.x);
                        const tileCenterZ = Math.round(this.player.z);
                        const centerSpeed = 3.0 * delta;
                        nextX = this.player.x + (tileCenterX - this.player.x) * centerSpeed;
                        nextZ = this.player.z + (tileCenterZ - this.player.z) * centerSpeed;
                        
                        // Also nudge slightly forward into the corridor to avoid re-triggering collision
                        const fwdNudge = 0.5 * delta;
                        nextX -= Math.sin(target) * fwdNudge;
                        nextZ -= Math.cos(target) * fwdNudge;
                    ...
                    if (this.mapData[pGridNextX] && this.mapData[pGridNextX][pGridNextZ] && this.mapData[pGridNextX][pGridNextZ].isShopDoor) {
                        if (this.player.shopDebt > 0) {
                            ...
                            if (this.player.gold >= this.player.shopDebt) {
                                ...
                                if (shopkeeperMesh) this.spawnChatBubble("Thank you for your business!", "Maido ari!", shopkeeperMesh);
                            } else {
                                // Block exit
                                nextX = this.player.x;
                                nextZ = this.player.z;
                                if (shopkeeperMesh) this.spawnChatBubble(`You need ${this.player.shopDebt - this.player.gold} more gold to leave with that!`, "Okaneganai!", shopkeeperMesh);
                ...
                if (this.worldGroup && currentlyMoving) {
                    ...
                    this.worldGroup.children.forEach(child => {
                        if (child.userData && child.userData.type === 'enemy') {
                            ...
                            // Keep circle black even when in active combat engagement
                        }
                    });
                }
                ...
                // AI gets to move if player is moving OR if the player injected action time by using a card.
                if (currentlyMoving || this.actionTimePool > 0) {
                    this.updateCombatAI(delta);
                    if (!currentlyMoving && this.actionTimePool > 0) {
                        this.actionTimePool -= delta;
                        if (this.actionTimePool < 0) this.actionTimePool = 0;
                    }
                }
                ...
                for (let bi = this.boulders.length - 1; bi >= 0; bi--) {
                    ...
                    if (b.userData.isBoulder && b.position.y <= 0.55) {
                        b.position.y = 0.55;
                        if (b.userData.vY < -1.5) {
                            // Hard bounce
                            b.userData.vY = Math.abs(b.userData.vY) * 0.8; // More bouncy pinball!
                            b.userData.vX *= 0.95;
                            b.userData.vZ *= 0.95;
                        } else {
                            // Rolling on floor
                    ...
                    if (bWallHit) {
                        if (b.userData.isBoulder) {
                            ...
                            // Spawn a tiny bit of dust/sparks on wall hit
                            this.spawnRockExplosion(b.position.clone());
                        } else {
                            // Missiles stick and disappear
                            this.spawnRockExplosion(b.position.clone());
                            this.scene.remove(b);
                            this.boulders.splice(bi, 1);
                            continue;
                    ...
                    if (this.worldGroup) {
                        for (let ci = 0; ci < this.worldGroup.children.length; ci++) {
                            const child = this.worldGroup.children[ci];
                            if (child.userData?.type === 'enemy' && !child.userData.isDead) {
                                ...
                                if (dist2D < 1.1) {
                                    if (b.userData.isShuriken || b.userData.type === 'SHURIKEN') {
                                        this.scene.remove(b);
                                        this.boulders.splice(bi, 1);
                                        
                                        const dmgMod = Math.min(1.0, b.userData.dmg / 50.0);
                                        const hitX = (Math.random() - 0.5) * (1.5 - dmgMod);
                                        const hitY = 0.5 + Math.random() * 1.5 + (dmgMod * 0.5);
                                        const hitZ = (Math.random() - 0.5) * (1.5 - dmgMod);
                                        
                                        b.position.set(hitX, hitY, hitZ);
                                        b.rotation.x += (Math.random() - 0.5);
                                        b.rotation.z += (Math.random() - 0.5);
                                        child.add(b);
                                        
                                        if (this.spawnCrosshair) this.spawnCrosshair(child, hitX, hitY, hitZ);
                ...
                if (this.lootItems && this.lootItems.length > 0) {
                    ...
                    for (let i = this.lootItems.length - 1; i >= 0; i--) {
                        ...
                        // Proximity Check (1.0 Tile Radius Auto-pickup)
                        const lX = loot.position.x / this.gridSize;
                        const lZ = loot.position.z / this.gridSize;
                        const distToLoot = Math.hypot(this.player.x - lX, this.player.z - lZ);
                                ...
                                if (loot.userData.cardDataURL) {
                                    ...
                                    pNode.innerHTML = `
                                        ...
                                        <div style="position:relative; display:inline-block; margin:0 auto;">
                                            ...
                                            <div id="loot3dOverlay" style="position:absolute; top:58%; left:50%; transform:translate(-50%, -50%); width:120px; height:120px; border-radius:50%; overflow:hidden;"></div>
                                        </div>
                                    `;
                                    ...
                                    // Render miniature 3D model inside the card overlay
                                    if (loot.userData.iconMesh) {
                                        const miniContainer = document.getElementById('loot3dOverlay');
                                        ...
                                        const miniLight = new THREE.AmbientLight(0xffffff, 1.5);
                                        const miniPoint = new THREE.PointLight(0xffffff, 2.0);
                                        miniPoint.position.set(2, 2, 3);
                                        miniScene.add(miniLight, miniPoint);
                                        
                                        const clonedIcon = loot.userData.iconMesh.clone();
                                        clonedIcon.position.set(0, 0, 0);
                                        clonedIcon.scale.set(1.5, 1.5, 1.5); // Ensure it looks big and shiny in the card
                                        miniScene.add(clonedIcon);
                                        
                                        let animFrame;
                                        const renderMini = (time) => {
                                            animFrame = requestAnimationFrame(renderMini);
                                            const t = time * 0.001;
                                            if (loot.userData.customUpdate) {
                                                loot.userData.customUpdate(t);
                                                // Sync cloned rotation back from the original, or run logic directly on clone
                                                // It's safer to just spin the whole group for the overlay:
                                                clonedIcon.rotation.y = t * 2.0;
                                                clonedIcon.rotation.x = Math.sin(t * 3.0) * 0.2;
                                            } else {
                                                clonedIcon.rotation.y += 0.05;
                                                clonedIcon.rotation.x = Math.sin(t * 3.0) * 0.1;
                                            }
                                            miniRenderer.render(miniScene, miniCamera);
                                        };
                                        renderMini(performance.now());
                ...
                if (this.topDownCamera) {
                    ...
                    if (activeRoom) {
                        // ROOM MODE: Precisely frame entire room + 1 tile border
                        const roomCenterX = (activeRoom.x + activeRoom.w / 2) * this.gridSize;
                        const roomCenterZ = (activeRoom.y + activeRoom.h / 2) * this.gridSize;
                        const roomSpan = Math.max(activeRoom.w, activeRoom.h) + 2; // +1 tile each side
                        const roomWorldSize = roomSpan * this.gridSize; // Total world units to fit
                        
                        // Physically scale camera placement to lower height and pull further back for a sharper Diablo-style angled perspective
                        targetHeight = roomWorldSize * 0.75 * this._pipZoomScale;
                        targetBack = roomWorldSize * 0.65 * this._pipZoomScale; 
                        
                        // Calculate exact FOV to fit roomWorldSize in frame natively given the new hypotenuse distance
                        const camDist = Math.sqrt(targetHeight * targetHeight + targetBack * targetBack);
                        targetFOV = 2 * Math.atan((roomWorldSize * 0.65) / camDist) * (180 / Math.PI);
                        
                        lookAtX = roomCenterX;
                        lookAtZ = roomCenterZ;
                    } else {
                        // Increase dynamic angle for hallways as well (lower height, pushed back)
                        targetFOV = 48;
                        targetHeight = 22 * this._pipZoomScale;
                        targetBack = 12 * this._pipZoomScale;
                        lookAtX = camWX;
                        lookAtZ = camWZ;
                    }
                    ...
                    this.topDownCamera.fov = this._pipFOV;
                    this.topDownCamera.aspect = aspect;
                    this.topDownCamera.updateProjectionMatrix();

                    // Initialize or use interactive PiP orbit controls
                    if (!this._pipRot) this._pipRot = { theta: 0, phi: 0.8 };
                    
                    // Lock PiP rotation to FPV to maintain proper alignment
                    this._pipRot.theta = this.player.rot;
                    
                    // Use directly smoothed height and back offsets for fluid zoom/pan
                    const camPosY = this._pipHeight;
                    const mapXZRadius = this._pipBack;
                    
                    const camPosX = this._pipOrthoTarget.cx + Math.sin(this._pipRot.theta) * mapXZRadius;
                    const camPosZ = this._pipOrthoTarget.cz + Math.cos(this._pipRot.theta) * mapXZRadius;

                    // Offset camera forward so player appears near the bottom of the screen (only in hallways)
                    const targetForwardOffset = activeRoom ? 0 : (8 * this._pipZoomScale);
                    if (this._pipForwardOffset === undefined) this._pipForwardOffset = targetForwardOffset;
                    this._pipForwardOffset += (targetForwardOffset - this._pipForwardOffset) * camTrackSpeed;
                    
                    const offX = -Math.sin(this._pipRot.theta) * this._pipForwardOffset;
                    const offZ = -Math.cos(this._pipRot.theta) * this._pipForwardOffset;
                    ...
                    if (this._pipLastInteractionTime && (Date.now() - this._pipLastInteractionTime > 1500)) {
                        if (this._pipPan) {
                            ...
                            // Clamp to exact zero to stop micro-calculations
                        ...
                        if (this._pipZoomScale !== undefined) {
                            this._pipZoomScale += (1.0 - this._pipZoomScale) * 0.05;
                        }
                    }
                    ...
                    this.topDownCamera.position.set(camPosX + offX + panX, camPosY, camPosZ + offZ + panZ);
                    this.topDownCamera.lookAt(this._pipOrthoTarget.cx + offX + panX, 0, this._pipOrthoTarget.cz + offZ + panZ);
                ...
                if (this.cameraMode === 'topdown' && this.topDownCamera) {
                    ...
                    // Lower ambient slightly from 1.6 to 0.7 so directional shadows actually render black, but keep visibility high
                    if (this.ambientLight) this.ambientLight.intensity = 0.7;
                    this.renderer.setClearColor(0x06020f, 1);
                    if (this.headlamp) this.headlamp.visible = false;
                    this.renderer.render(this.scene, this.topDownCamera);
                    if (this.headlamp) this.headlamp.visible = true;
                    ...
                    if (this.topDownCamera) {
                        ...
                        this.scene.fog.density = 0.02;
                        if (this.ambientLight) this.ambientLight.intensity = 0.7; // Lowered from 1.6 to allow solid shadows to express
                        this.renderer.setClearColor(0x000000, 1); // True pitch black void
                        if (this.headlamp) this.headlamp.visible = false;
                        this.renderer.render(this.scene, this.topDownCamera);
                        if (this.headlamp) this.headlamp.visible = true;
                        if (this.ambientLight) this.ambientLight.intensity = 0.5; // Restore FPV cinematic gloom
