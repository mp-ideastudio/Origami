    ...
    <style>
        ...
        #encounter-zone { position: absolute; top: 50%; left: 0; width: 100%; transform: translateY(-50%); display: flex; justify-content: space-between; padding: 0 20px; box-sizing: border-box; pointer-events: none; }
        .side-panel { 
        ...
        @media (max-width: 1100px) {
            #bottom-panel, #input-container, #event-log-container { width: calc(var(--card-w) * 5 + 56px); }
            .card-column:nth-child(n+6) { display: none !important; }
        }
        @media (max-width: 900px) {
            #bottom-panel, #input-container, #event-log-container { width: calc(var(--card-w) * 4 + 42px); }
            .card-column:nth-child(n+5) { display: none !important; }
        }
        @media (max-width: 750px) { 
            #bottom-panel, #input-container, #event-log-container { width: calc(var(--card-w) * 3 + 28px); }
            .card-column:nth-child(n+4) { display: none !important; }
            #left-panel { transform: scale(0.85); transform-origin: left center; }
            #right-panel { transform: scale(0.85); transform-origin: right center; }
        }
        @media (max-width: 500px) { 
            #bottom-panel, #input-container, #event-log-container { width: calc(var(--card-w) * 3 + 28px); }
            .card-column:nth-child(n+4) { display: none !important; }
            #left-panel { transform: scale(0.75); transform-origin: left center; }
            #right-panel { transform: scale(0.75); transform-origin: right center; }
        }
        ...
        .guide-card {
            ...
            padding: 0; box-sizing: border-box; border: 1px solid rgba(255,255,255,0.15);
        ...
        .chat-pill { min-width: 0; width: 100%; height: 54px; background: var(--bg-glass);   box-shadow: 2px 2px 8px var(--nm-shadow-dark); border-radius: 12px; display: flex; align-items: center; padding: 0 18px; border: var(--glass-border); box-sizing: border-box; margin: 0; }
        .chat-input { flex: 1; width: 10px; background: transparent; border: none; outline: none; color: #FFF; font-family: 'Nunito', sans-serif; font-weight: 900; font-size: 16px; text-shadow: none; min-width: 0; text-overflow: ellipsis; overflow: hidden; white-space: nowrap; }
    ...
    <div id="ui-dock">
        ...
        <div id="encounter-zone">
            ...
            <div id="pax-console" class="side-panel">
                <div class="panel-toggle toggle-left" id="toggle-pax" onclick="toggleSidePanel('pax-console', 'toggle-pax', true)">&lt;</div>
            ...
            <div id="action-rail" class="side-panel">
                <div class="panel-toggle toggle-right" id="toggle-rail" onclick="toggleSidePanel('action-rail', 'toggle-rail', false)">&gt;</div>
                ...
                <div class="sp-sub sp-xp-status">XP: <span id="player-xp-val">0</span> / <span id="player-xp-max">100</span></div>
        ...
        <style>
            ...
            #encounter-zone.active #pax-console.minimized { transform: translateX(calc(-100% - 20px)); }
            #encounter-zone.active #action-rail.minimized { transform: translateX(calc(100% + 20px)); }
            ...
            .event-toggle:hover { background: rgba(255, 255, 255, 0.15); }
            #event-log-container.active ~ #input-container .event-toggle i::before { content: "\f068"; }
            ...
            #event-log-container.active ~ #input-container .chat-pill {
            ...
            #mapview-container {
                ...
                display: none; /* Only visible in dock mode */
            }

            #mapview-container:hover {
                border-color: rgba(200,150,60,0.35);
            }

            #mapview-container.dragging {
                cursor: grabbing !important;
                z-index: 10005 !important;
            }

            #mapview-container.resizing {
                transition: none;
            }

            #mapview-container:active {
                cursor: grabbing;
            }

            #mapview-container .canvas-wrapper {
                position: relative;
                width: 100%;
                height: 100%;
                overflow: hidden;
            ...
            #mapview-container .canvas-wrapper::after {
                content: '';
                position: absolute;
                top: 0; left: 0; right: 0; bottom: 0;
                ...
                -webkit-mask-image: radial-gradient(circle at 50% 50%, transparent 40%, black 80%);
                mask-image: radial-gradient(circle at 50% 50%, transparent 40%, black 80%);
                pointer-events: none;
                z-index: 10;
            }

            #mapview-container .canvas-wrapper canvas {
                display: block;
                width: 100%;
                height: 100%;
                pointer-events: auto;
            ...
            .resize-handle.se { bottom: -2px; right: -2px; cursor: se-resize; border-bottom: 2px solid rgba(255,255,255,0.3); border-right: 2px solid rgba(255,255,255,0.3); border-bottom-right-radius: 8px;}
            .resize-handle.sw { bottom: -2px; left: -2px; cursor: sw-resize; border-bottom: 2px solid rgba(255,255,255,0.3); border-left: 2px solid rgba(255,255,255,0.3); border-bottom-left-radius: 8px;}
            .resize-handle.ne { top: -2px; right: -2px; cursor: ne-resize; border-top: 2px solid rgba(255,255,255,0.3); border-right: 2px solid rgba(255,255,255,0.3); border-top-right-radius: 8px;}
            .resize-handle.nw { top: -2px; left: -2px; cursor: nw-resize; border-top: 2px solid rgba(255,255,255,0.3); border-left: 2px solid rgba(255,255,255,0.3); border-top-left-radius: 8px;}

            #mapview-container .view-label {
                position: absolute;
                top: 6px;
                left: 50%;
                transform: translateX(-50%);
        ...
        <div id="lower-ui-row">
            ...
            <div id="right-panel" class="u-wing u-panel" tabindex="3">
                <div id="action-zone">
                    <div class="level-badge" id="player-level-badge">1</div>
        ...
        function playAvatarAnim(name) {
            ...
            if (btn) {
                ...
                if (isMoving) btn.classList.add('anim-walk-btn');
            }
            ...
            categories.forEach((cat, idx) => {
                ...
                const endDrag = (e) => {
                    ...
                    if (!dragMoved) {
                        ...
                        if (clickedCard && clickedCard.parentElement === col) {
                            setFocus(idx);
                            if(clickedCard.dataset.depth === "0") {
                                if (clickedCard.classList.contains('selected-card')) {
                                    ...
                                    setTimeout(() => { clickedCard.classList.remove('launching'); }, 450);
                                } else {
                                    // First click: Select!
                ...
                cat.cards.forEach((name, i) => {
                    ...
                    card.className = `guide-card card-${(cat.id === 'KATANA' || cat.id === 'SHIELD' || cat.id === 'MISSILE') ? 'item' : cat.id.toLowerCase()}`;
...
// (Dojo Shoji Texture removed; reverting side-panels to standard glass texture to match playing cards)
    ...
    (function() {
        ...
        function syncRect() {
            const wrapper = pip.querySelector('.canvas-wrapper');
            if (wrapper) {
        ...
        pip.addEventListener("contextmenu", e => { 
            if (e.target.closest(".canvas-wrapper") || e.target.closest(".view-label")) e.preventDefault(); 
        });
        ...
        function loadPiPState() {
            ...
            try {
                const raw = localStorage.getItem("mapPiPState.v2");
                if (!raw) return;
                const s = JSON.parse(raw);
                if (!s || typeof s !== "object") return;
                pip.style.position = "absolute";
                if (typeof s.left === "number") pip.style.left = s.left + "px";
        ...
        function savePiPState() {
            if (window.innerWidth <= 768) return;
            const rect = pip.getBoundingClientRect();
            try { localStorage.setItem("mapPiPState.v2", JSON.stringify({ left: rect.left, top: rect.top, width: rect.width, height: rect.height })); } catch {}
        }
        ...
        const onPanEnd = (e) => { 
            ...
            if (Math.hypot(e.clientX - panStart.x, e.clientY - panStart.y) < 5) {
                const rect = pip.querySelector('.canvas-wrapper').getBoundingClientRect();
